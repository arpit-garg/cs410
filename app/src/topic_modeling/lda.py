...
class LDA(object):
    ...
    def gibbsSamplingUpdateDWK(self, d, w, k):
        terms1 = map(lambda i: self.Alpha[i] + self.queryDocTopic(d, i, d, w, k), range(0, self.K));
        terms2 = map(lambda i: self.Beta[w] + self.queryWordTopic(w, i, d, w, k), range(0, self.K));
        terms3 = map(lambda i: self.BetaSum + self.queryTopic(i, d, w, k), range(0, self.K));
        propProb = map(lambda i: terms1[i] * terms2[i] / terms3[i], range(0, self.K));
        pdf = map(lambda i: propProb[i] / sum(propProb), range(0, self.K));
