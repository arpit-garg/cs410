...
window.getData = (cb) => {
  // var temp = 'topics';
  // d3.request("http://127.0.0.1:5000/topics")
    // .get(error,data) => {
  $.get("http://127.0.0.1:5000/topics", function(data, status){
    console.log(data);
    console.log(status);
    ...
    for (var i in data){
    //console.log(data[i])
    var topics = data[i].topicTitle.split(" ")
      for (var j = topics.length -1; j >= 5; j--)
        {topics.splice(topics[j],1);}
    data[i].topicTitle = topics
    //console.log(data[i].topicTitle)
    }
    ...
    nodes.forEach((node, nodeIdx) => {
      ...
      if (node.type !== 'year-node') {
        ...
        if (yearIndex > 0 && yearIndex !== allYears.length - 1) {
          prevAvailableYear = allYears[yearIndex - 1]
          prevAvailableYear = nextAvailableYear.toString().replace(/\./g,'-')
        ...
        if (yearIndex < allYears.length - 1 && yearIndex !== -1) {
          ...
          // Mapping to the prev year
          nextAvailableYear = allYears[yearIndex + 1]
          nextAvailableYear = nextAvailableYear.toString().replace(/\./g,'-')
    ...
    const timeLineNodes = allYears.map((e) => {
      tempe = e.toString().replace(/\./g,'-')
      const yearNode = {
        timeSort: `timeline-year-${tempe}`,
        topic: tempe.toString(),
        name: tempe.toString(),
        year: e,
        topicTitle: '',
        influencedBy: [],
    ...
    console.log('Returning nodes and links')
