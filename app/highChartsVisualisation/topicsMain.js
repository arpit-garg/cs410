...
function plot(svg, width, height, data) {
  ...
  console.log('Initialising sankey')
  sankey
    .nodes(data.nodes)
    .links(data.links)
    .layout(32)

  console.log('Sankey initialised')

  // Tooltips
  const tip = d3.tip()
    .attr('class', 'd3-tip')
    .offset([-10, 0])
    .html(d => `Topics: ${d.topicTitle}`)
  svg.call(tip)
...
window.getData((data) => {
  ...
  console.log('Plotting nodes data')
  plot(moviesSVG, width, height, {
    nodes: data.nodes,
    links: data.links,
  })

  console.log('Plotting timeline data')
